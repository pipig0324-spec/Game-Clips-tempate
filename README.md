# Game Clips Template

Configurable 16:9 announcement-video template for Game Clips.

## Features

- Editable announcement and Discord profile fields
- Configurable typography, colors, and logo size
- Draggable clip calibration with precise position and size controls
- Local 1920x1080 video export with clip audio
- Browser-local configuration persistence

All uploaded clips and avatars remain in the browser and are not sent to a server.

## Local preview

Serve this directory with any static HTTP server, then open `index.html`.

```powershell
python -m http.server 4173
```

